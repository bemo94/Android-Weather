...
import static com.octo.usecases.StringUtils.isNotBlank;
...
public class CityForecastInteractor {
    ...
    private void handleRetrievedForecast(CityWeeklyForecast cityForecast) {
        ...
        if (relevantForecasts.size() >= REQUIRED_FORECASTS) {
            ...
            presenter.onForecasts(
                cityForecast.getCityName(),
                worstForecast(relevantForecasts),
                bestForecast(relevantForecasts)
            );
        } else {
